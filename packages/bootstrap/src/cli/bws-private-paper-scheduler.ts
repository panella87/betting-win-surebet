...
export function printBwsPrivatePaperSchedulerHelp(stream: NodeJS.WriteStream = process.stdout): void {
  stream.write(
    [
      ...
      '',
      'Runs one bounded BWS private-paper scheduler pass for the explicit api or export upstream mode. It advances explicit upstream convergence, persists restart-safe scheduler checkpoints, and enqueues deterministic private-paper worker jobs without fallback.',
      'Required environment: BETTING_WIN_REPO_PATH, BWS_UPSTREAM_LOCK_PATH, BWS_UPSTREAM_MODE, BWS_PRIVATE_PAPER_SCHEDULE_PATH, BWS_WORKER_QUEUE_NAME, SUREBET_RUNTIME_MODE=paper, SUREBET_PROVIDER_CONNECTIONS=disabled, SUREBET_EXECUTION_ENABLED=false, SUREBET_PG_DATABASE, SUREBET_PG_USER, SUREBET_PG_PORT, and exactly one of SUREBET_PG_HOST or SUREBET_PG_SOCKET_DIRECTORY.',
