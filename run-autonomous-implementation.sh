...
run_validation() {
  local label="$1" out_dir="$2" log_file="$out_dir/${label}_validation.log"
  mkdir -p "$out_dir"
...
write_cycle_context() {
  local cycle_dir="$1" context_file="$cycle_dir/CYCLE_CONTEXT.md"
  {
    echo "# Cycle Context"
    echo
...
write_cycle_prompt() {
  local cycle_dir="$1" cycle_num="$2" context_file="$3" prompt_file="$cycle_dir/codex_prompt.txt"
...
run_codex_cycle() {
  local cycle_dir="$1" prompt_file="$cycle_dir/codex_prompt.txt" log_file="$cycle_dir/codex.log" rc
...
main() {
  ...
  if [[ "$ALLOW_PARALLEL" != "1" ]]; then
    if ! mkdir "$LOCK_DIR" 2>/dev/null; then
      ...
      FINAL_STATUS="BLOCKED=yes"
      echo "Existing repo-scoped autonomous controller detected." > "$RUN_DIR/parallel_repo_automation_detected.txt"
      finish 2
    fi
  fi
