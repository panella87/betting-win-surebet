...
ACTIVE_HANDOFF_PAPER_SERVICE_SUPPORTED="0"
ACTIVE_HANDOFF_SERVICE_REFRESH_REQUIRED="0"
ACTIVE_HANDOFF_RUNTIME_EVIDENCE_REQUIRED="0"
ACTIVE_HANDOFF_RUNTIME_EVIDENCE_SELECTED_UPSTREAM_MODE="none"
ACTIVE_HANDOFF_RUNTIME_EVIDENCE_CAMPAIGN_RUN_ID="none"
...
load_active_handoff() {
  ...
  if [[ "$ACTIVE_HANDOFF_MODE" == "paper" ]]; then
    ...
    ACTIVE_HANDOFF_PAPER_SERVICE_SUPPORTED="$(require_handoff_zero_one PAPER_SERVICE_SUPPORTED)" || return 2
    ACTIVE_HANDOFF_SERVICE_REFRESH_REQUIRED="$(require_handoff_zero_one SERVICE_REFRESH_REQUIRED)" || return 2
    ACTIVE_HANDOFF_RUNTIME_EVIDENCE_REQUIRED="$(require_handoff_zero_one RUNTIME_EVIDENCE_REQUIRED)" || return 2
    ACTIVE_HANDOFF_RUNTIME_EVIDENCE_SELECTED_UPSTREAM_MODE="$(automation_v2_env_require RUNTIME_EVIDENCE_SELECTED_UPSTREAM_MODE)" || return 2
    ACTIVE_HANDOFF_RUNTIME_EVIDENCE_CAMPAIGN_RUN_ID="$(automation_v2_env_require RUNTIME_EVIDENCE_CAMPAIGN_RUN_ID)" || return 2
    if [[ "$ACTIVE_HANDOFF_RUNTIME_EVIDENCE_REQUIRED" == "1" ]]; then
      [[ "$ACTIVE_HANDOFF_PAPER_SERVICE_SUPPORTED" == "1" && "$ACTIVE_HANDOFF_SERVICE_REFRESH_REQUIRED" == "1" ]] || {
      ...
      [[ "$ACTIVE_HANDOFF_RUNTIME_EVIDENCE_CAMPAIGN_RUN_ID" == "none" || "$ACTIVE_HANDOFF_RUNTIME_EVIDENCE_CAMPAIGN_RUN_ID" =~ ^[A-Za-z0-9._:-]+$ ]] || {
        echo "ERROR: invalid RUNTIME_EVIDENCE_CAMPAIGN_RUN_ID" >&2
        return 2
      }
    else
      [[ "$ACTIVE_HANDOFF_PAPER_SERVICE_SUPPORTED" == "0" && "$ACTIVE_HANDOFF_SERVICE_REFRESH_REQUIRED" == "0" ]] || {
        echo "ERROR: no-service paper handoff requires PAPER_SERVICE_SUPPORTED=0 and SERVICE_REFRESH_REQUIRED=0" >&2
        return 2
      }
      [[ "$ACTIVE_HANDOFF_RUNTIME_EVIDENCE_SELECTED_UPSTREAM_MODE" == "none" ]] || return 2
      [[ "$ACTIVE_HANDOFF_RUNTIME_EVIDENCE_CAMPAIGN_RUN_ID" == "none" ]] || return 2
    fi
