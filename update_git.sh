...
Environment (.env supported):
  GITHUB_TOKEN  Optional PAT used for non-interactive HTTPS auth. Never print it.
USAGE
}
...
run_git_with_auth() {
  if [[ -n "${GITHUB_TOKEN:-}" ]]; then
    local hdr
    hdr="$(auth_header)"
    git -c http.https://github.com/.extraheader="$hdr" "$@"
    return $?
  fi
  git "$@"
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --clone)
      ACTION="clone"; shift
      [[ $# -ge 1 ]] || { echo "Error: --clone requires OWNER/REPO" >&2; exit 1; }
      CLONE_SLUG="$1"; shift
      if [[ $# -gt 0 ]]; then TARGET_DIR="$1"; shift; fi
      ;;
