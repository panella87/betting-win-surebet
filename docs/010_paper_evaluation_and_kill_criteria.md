...
BWS private paper evaluation may consume only an accepted, authorized, contract-compatible betting-win downstream read-only API for runtime evidence; the current cross-repository handoff is not yet accepted. Immutable exports and explicit pinned bundles remain deterministic fixture, parser, and backtest compatibility inputs only; they are not selectable runtime transports. Direct provider access and execution remain prohibited.
