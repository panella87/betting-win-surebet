...
4. Keep the private BWS `.env` configured for PostgreSQL, the accepted betting-win read-only API, and `BWS_PRIVATE_PAPER_SCHEDULE_PATH` pointing to an operator-approved repo-local manifest under `runtime/operator-inputs/`. Explicit shell values win; the runtime wrapper fills only missing approved non-policy keys from `.env` and enforces API mode, paper mode, provider-disabled operation, and execution-disabled operation. It never substitutes a fixture schedule.
