...
The controller must maximize cumulative safe implementation from `BWS-520` through `BWS-580`, validating and updating the dependency ledger after each coherent slice.

Do not run paper autopilot while this source queue remains. Do not enable protected automation changes unless a later explicit automation-maintenance task identifies an exact allowlist.
