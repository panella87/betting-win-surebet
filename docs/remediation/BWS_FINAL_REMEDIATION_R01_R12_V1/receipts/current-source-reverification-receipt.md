...
# current-source-reverification-receipt

Schema: `../schemas/current-source-reverification-receipt.schema.json`
Non-evidence example: `../examples/current-source-reverification-receipt.example.json`

## Purpose

This immutable receipt records the `current-source-reverification-receipt` boundary for `BWS_FINAL_REMEDIATION_R01_R12_V1`. Unknown fields are rejected. Missing required fields fail closed. Raw secrets, credentials, passwords, tokens, private keys, or protected configuration values are prohibited.
