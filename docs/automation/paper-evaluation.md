...
artifacts_zip=root_artifacts.zip
adaptive_flag=--adaptive
adaptive_interval_clamp=5m..60m
...
It is not the selected implementation controller and cannot prove continuous `BWS-600` readiness on its own. It may validate repo source, run bounded private fixture or explicit repo-local pinned-bundle checks, or collect bounded runtime evidence with exact stack ownership checks. The explicit pinned-bundle variable remains `SUREBET_PINNED_BUNDLE`.
