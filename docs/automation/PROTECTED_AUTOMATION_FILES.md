...
Do not set `AUTOMATION_ALLOW_PROTECTED_CHANGES=1` for the current `BWS-700` implementation state, the carry-forward `BWS-600` runtime-evidence state, or any ordinary implementation, paper, or bugfix run. Do not broaden authorization from inside an autonomous cycle.
