...
Normal `--max-rounds 0` means the parent duration and repeated bug-signature guard control termination. The parent clamps each child duration to the remaining parent budget.
 The seven-day and 72-hour values are ceilings: completed bounded work advances immediately to same-area re-audit and the next campaign area rather than sleeping to consume the configured time.
...
The controller writes `artifacts/bugfix_autopilot_*`, `campaign_coverage.tsv`, `rounds.tsv`, per-round child command/output/result files, final summaries, `artifacts.zip`, and one final parent Telegram notification. Before ZIP publication, the shared artifact-residue policy removes only explicit test/release scratch families; all campaign run directories and referenced child evidence remain retained. Every child launch records and receives `TELEGRAM_NOTIFY=0`. Audit and implementation handoffs use strict schema-v1 allowlists, evidence hashes, semantic fingerprints, and an atomic child-result side channel with exact parent/child identity, exit-code, run-directory, and lock-state reconciliation. Machine output includes `child_cleanup_status`, `child_cleanup_exit_code`, `lock_release_status`, `lock_release_exit_code`, and `lock_preserved`. Active-child identity/termination failure preserves the lock. Strict release failure becomes `BUGFIX_AUTOPILOT_BLOCKED_LOCK_RELEASE` with exit code `2`; the final summary and `artifacts.zip` are corrected before Telegram. Verified force-unlock uses zombie-aware checks, TERM-first process-group termination, bounded KILL escalation, and post-KILL death verification before any lock removal.
...
## In-campaign protected-controller transition

A bugfix implementation child may be explicitly authorized to update protected controller helpers while its parent remains alive. The parent retains its already-loaded functions until it exits, while the next child reads the updated files. The transition is therefore handled as a one-generation compatibility boundary, not as a general legacy-lock fallback.

The updated child accepts an identity-less parent lock only when every condition is true: the lock is a non-symlink regular file at the exact `AUTOMATION_PARENT_LOCK_FILE`, its PID is the child process's direct parent, the declared parent controller/PID match the environment, the repository and script paths are canonical, the parent command line names that exact script, and the parent/child controller pair is explicitly supported. Unrelated, skipped, mismatched, or partially identified live locks remain fatal. New parent processes always emit and require boot ID plus process-start ticks.

When the direct parent predates the identity-rich child-result schema, the child emits the exact legacy terminal-result key set that the already-running parent can validate. New parents receive the identity-rich result. A new parent may also receive `RUN_DIR=none`, but only for `setup_failed` / `unexpected_exit_before_start`, zero completed cycles, and a nonzero exit code. This preserves a machine-readable terminal result when a child fails before creating its run directory without weakening canonical run-directory checks for ordinary results.
