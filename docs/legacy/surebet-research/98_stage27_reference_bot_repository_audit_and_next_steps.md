...
> **Historical reviewed research checkpoint.** The Prompt 28 synthesis requested by this file was completed and is retained under `research/imported-from-betting-win/legacy/surebet/synthesis/`. This file is not a current command or implementation queue.
...
historical_next_step_status=COMPLETED_BY_PROMPT28_SYNTHESIS
```
...
- All terminal settlement scenarios, including push, void, cancel, and retirement, belong in the payoff matrix.

## Security decision

`polymarket-copy-bot-main` and `polymarket-trading-bot-master` are quarantined. Do not install or run them. The first passes a private key into a nonstandard method from a dependency resolved through a nonstandard tarball host. The second has an undeclared import and dependency mismatch and a remote-value process gate unrelated to the strategy.

## Verification

- 63 JavaScript files across four non-quarantined or static repositories passed `node --check`.
...
The previously referenced Prompt 28 source path is not part of this repository and is no longer an action. Its completed outputs are:
...
Current implementation and acceptance authority is `docs/047_b1_cross_venue_offline_falsification_program.md` through `docs/052_b1_future_strategy_stubs.md`.
