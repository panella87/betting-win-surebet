...
For `BWS_FULL_PLATFORM_IMPLEMENTATION_V1`, use `CONTINUE_REQUIRED=yes` while a dependency-ready safe task through `BWS-510` remains. Use `AUTONOMOUS_GOAL_COMPLETE=yes` only when every safe local task is `VALIDATED`. Historical SURE completion is not an active stop condition.
