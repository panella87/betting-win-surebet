...
test('status docs record the hardened controller surface', () => {
  ...
  assertContains(status, 'run_autonomous_implementation=available_for_future_reviewed_source_handoff_or_unblocked_bws710');
  ...
  assertContains(status, 'run_paper_evaluation=fixture_and_runtime_evidence_validated_bws_588');
  assertContains(status, 'run_paper_autopilot=standardized_selected_for_bws600_runtime_evidence_after_bws700_local_completion');
  assertContains(status, 'paper_autopilot=selected_after_bws700_dependency_ready_queue_complete');
