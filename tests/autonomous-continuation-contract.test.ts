...
test('autonomous authority records the completed safe-local operator-runtime queue', () => {
  ...
  assert.match(status, /run_autonomous_implementation=selected_for_bws600_upstream_api_preflight_source_fix/);
  assert.match(status, /run_paper_autopilot=paused_until_bws600_upstream_api_preflight_source_fix_validated/);
