...
test('upstream API convergence rejects recovered import-run provenance that does not match the verified lock', async () => {
  for (const provenanceOverride of [
    Object.freeze({ commitSha: '9'.repeat(40) }),
    Object.freeze({ repository: 'other-repository' }),
    Object.freeze({ sourceView: 'workspace' }),
    Object.freeze({ verifiedAt: '2026-07-15T13:00:06.000Z' }),
  ] as const) {
    const fixture = createApiFixture({
      identity: [createEnvelope('identity', { page: { items: [createIdentityItem()], pageSize: 1, returnedCount: 1 } })],
      rules: [createEnvelope('rules', { page: { items: [createRulesItem()], pageSize: 1, returnedCount: 1 } })],
    ...
    try {
      ...
      assert.equal(result.blockers[0]?.code, 'BWS_UPSTREAM_API_IMPORT_PROVENANCE_MISMATCH');
      assert.equal(fixture.requestLog.length, 0);
