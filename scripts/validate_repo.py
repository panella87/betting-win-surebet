...
def main() -> None:
    ...
    required_doc_markers = {
        'README.md': ['program=BWS_FULL_PLATFORM_IMPLEMENTATION_V1', 'repo_role=surebet_strategy_application', 'current_task=BWS-600', 'safe_local_terminal_gate=BWS-599', 'run-paper-autopilot.sh'],
        'AGENTS.md': ['Source-of-truth order', 'BETTING_WIN_REPO_PATH', 'backlog/bws_full_implementation.csv', 'BWS-580', 'BWS-581', 'BWS-599'],
        'docs/automation/README.md': ['current_task=BWS-600', 'selected_controller=run-autonomous-implementation.sh', 'safe_local_terminal_gate=BWS-599', 'Exact protected-file policy', 'automation_maintenance_allowed=no', 'blanket manual override is disabled', 'repository-temp-inode-safety.md'],
        ...
        'PROJECT_STATUS.md': ['program=BWS_FULL_PLATFORM_IMPLEMENTATION_V1', 'status=RUNTIME_EVIDENCE_SOURCE_FIX_READY', 'current_task=BWS-600', 'current_task_status=SOURCE_IMPLEMENTATION_REQUIRED_BETTING_WIN_API_PREFLIGHT', 'safe_local_completion_gate=BWS-599', 'selected_controller=run-autonomous-implementation.sh'],
        'docs/repo_status_current.md': ['Standard automation status', 'run_autonomous_implementation=selected_for_bws600_upstream_api_preflight_source_fix', 'run_paper_autopilot=paused_until_bws600_upstream_api_preflight_source_fix_validated', 'run_bugfix_autopilot=standardized_parent_for_broad_audit_and_repair', 'paper_runtime_env_loader=selective_root_wrapper_env', 'paper_runtime_env_precedence=explicit_process_then_dotenv_fill', 'paper_runtime_schedule=operator_approved_repo_local_manifest', 'paper_runtime_policy=enforced_api_paper_provider_disabled_execution_false', 'source_fingerprint_runtime_exclusion=enabled', 'runtime_evidence_failure_stage=bounded_redacted', 'selected_task_source=docs/automation/current-implementation-task.md'],
