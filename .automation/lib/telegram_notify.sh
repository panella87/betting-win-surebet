...
telegram_notify_next_action() {
  ...
  case "$normalized" in
    ...
    BUGFIX_AUTOPILOT_BLOCKED_LOCK_RELEASE) printf 'Inspect the preserved bugfix-parent lock ownership and release evidence; do not start another controller until the lock is resolved.' ;;
